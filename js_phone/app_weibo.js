...
var feedHTML=wbData.feed.map(function(p,i){
    ...
    var safeAvatar = wbEsc(p.avatar||displayName[0]||'网');
    var safeTime = wbEsc(p.time||'刚刚');
    var cmtCount = Array.isArray(p.comments) ? p.comments.length : (p.commentCount||p.comments||0);
    return ''+ ''+safeAvatar+''+displayName+(p.verified?' ':'')+''+ safeTime +''+ ''+wbSafeContent(p.text||p.content||'')+''+wbImgs(p.imgs||[])+ ''+WB_IC.repost+' '+wbFmtNum(p.reposts||p.shares||0)+''+WB_IC.comment+' '+cmtCount+''+(p.liked?WB_IC.likeFilled:WB_IC.like)+' '+wbFmtNum(p.likes||0)+''+ '';
}).join('');
...
var list=wbData.hotSearch.map(function(h,i){
    var rc=i<3?'':' normal';
    var safeTag=h.tag?''+wbEsc(h.tag)+'':'';
    // ★ 热搜标题转义
    return ''+(i+1)+''+wbEsc(h.text||h.title)+''+safeTag+''+wbEsc(h.count||'')+'';
}).join('');
...
var showCount=wbData._cmtShowCount||5;var visibleCmts=allCmts.slice(0,showCount);
var cmts=visibleCmts.map(function(c){
    // ★ 评论区所有AI字段转义
    return ''+WB_IC.avatar+''+wbEsc(c.name||c.author)+''+wbSafeContent(c.text||c.content)+''+ wbEsc(c.time||'刚刚')+''+WB_IC.likeSm+''+(c.likes||0)+'';
}).join('');

if(allCmts.length>showCount){cmts+='<div onclick="wbLoadMoreComments()" style="text-align:center;padding:14px 0;font-size:13px;color:#ff6b2b;cursor:pointer;">展开更多评论 (剩余'+(allCmts.length-showCount)+'条)</div>';}
        return '<div class="weibo-container"><div class="wb-detail-nav"><div class="wb-detail-back" onclick="wbNav(\''+backTo+'\')">'+IC.back+'</div><div class="wb-detail-title">正文</div></div><div class="wb-body"><div class="wb-post" style="margin:0;"><div class="wb-post-header"><div class="wb-post-avatar" style="background:'+(p.color||'#ff6b2b')+';color:#fff;font-size:14px;" onclick="wbNav(\'profile\','+wbData.currentPostIdx+')">'+wbEsc(p.avatar||p.name[0]||'网')+'</div><div class="wb-post-info"><div class="wb-post-name" onclick="wbNav(\'profile\','+wbData.currentPostIdx+')">'+wbEsc(p.name||p.author)+(p.verified?' <span class="wb-post-vip"><svg width="8" height="8" viewBox="0 0 10 10" fill="none"><path d="M3 5l2 2 3-3" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></span>':'')+'</div><div class="wb-post-meta">'+wbEsc(p.time||'刚刚')+'</div></div></div><div class="wb-post-content">''+wbSafeContent(p.text||p.content||'')+''+wbImgs(p.imgs||[])+'<div class="wb-post-actions"><button class="wb-action-btn">'+WB_IC.repost+' '+wbFmtNum(p.reposts||p.shares||0)+'</button><button class="wb-action-btn">'+WB_IC.comment+' '+((p.comments||[]).length||0)+'</button><button class="wb-action-btn'+(p.liked?' liked':'')+'" onclick="wbLike('+wbData.currentPostIdx+',this)">'+(p.liked?WB_IC.likeFilled:WB_IC.like)+' <span>'+wbFmtNum(p.likes||0)+'</span></button></div></div><div class="wb-comments-section"><div class="wb-comments-tabs"><span class="wb-comments-tab active">热门</span><span class="wb-comments-tab">最新</span></div>'+(cmts||'<div style="text-align:center;padding:20px;color:#999;font-size:12px;">暂无评论</div>')+'</div></div><div class="wb-comment-input"><input id="wb-cmt-input" placeholder="写评论..." onkeydown="if(event.key===\'Enter\')wbComment()"><button class="wb-comment-send" onclick="wbComment()">发送</button></div></div>';
}

function renderWbSearch(){
    var hots = (wbData.hotSearch||[]).slice(0,6).map(function(h){
        return '<div style="padding:6px 12px;background:#f5f5f5;border-radius:14px;font-size:12px;color:#ff6b2b;cursor:pointer;">'+wbEsc(h.text||h.title)+'</div>';
    }).join('');
    var accList = (wbData.accounts||[]).slice(0,3).map(function(a){
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:0.5px solid #f5f5f5;cursor:pointer;" onclick="wbNav(\'account\',\''+a.id+'\')"><div style="width:36px;height:36px;border-radius:50%;background:'+a.color+';display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;">'+wbEsc(a.avatar)+'</div><div style="flex:1;"><div style="font-size:14px;color:#1a1a1a;font-weight:500;">'+wbEsc(a.name)+'</div><div style="font-size:11px;color:#999;margin-top:2px;">'+wbEsc((a.desc||'').split('·')[0].trim())+'</div></div><div style="font-size:11px;color:#bbb;">'+(a.followers||0)+'粉丝</div></div>';
    }).join('');
    ...
    if(r.accounts.length > 0){
        accHTML = '<div style="font-size:13px;color:#999;padding:12px 16px 6px;">用户</div>' +
            r.accounts.map(function(a){
                ...
                return '<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:0.5px solid #f5f5f5;cursor:pointer;" onclick="wbNav(\'account\',\''+a.id+'\')"><div class="wb-post-avatar" style="background:'+a.color+';color:#fff;font-size:14px;">'+wbEsc(a.avatar)+'</div><div style="flex:1;"><div style="font-size:14px;font-weight:600;color:#1a1a1a;">'+wbEsc(a.name)+vBadge+'</div><div style="font-size:12px;color:#999;margin-top:2px;">'+wbEsc(a.desc)+'</div></div><div style="padding:4px 12px;border:1px solid #ff6b2b;border-radius:14px;font-size:12px;color:#ff6b2b;cursor:pointer;">关注</div></div>';
            }).join('');
    }
    var postHTML = '';
    if(r.posts.length > 0){
        postHTML = '<div style="font-size:13px;color:#999;padding:12px 16px 6px;">相关微博</div>' +
            r.posts.map(function(p,i){
                ...
                return '<div class="wb-post" onclick="wbNav(\'detail\','+idx+')" style="margin:0;border-bottom:0.5px solid #f5f5f5;"><div class="wb-post-header"><div class="wb-post-avatar" style="background:'+(p.color||'#ccc')+';color:#fff;font-size:14px;">'+wbEsc(p.avatar||p.name[0]||'网')+'</div><div class="wb-post-info"><div class="wb-post-name">'+wbEsc(p.name||p.author)+'</div><div class="wb-post-meta">'+wbEsc(p.time||'刚刚')+'</div></div></div><div class="wb-post-content" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">''+wbEsc((p.text||p.content||'').substring(0,80))+'';'+'</div></div>';
            }).join('');
    }
...
function renderWbDmList(){
    ...
    var list=wbData.dms.map(function(d){
        return '<div class="wb-dm-item" onclick="wbNav(\'dm\',\''+d.id+'\')"><div class="wb-dm-avatar" style="background:'+d.color+'">'+wbEsc(d.avatar)+'</div><div class="wb-dm-info"><div class="wb-dm-name">'+wbEsc(d.name)+'</div><div class="wb-dm-preview">'+wbEsc(d.lastMsg)+'</div></div><div class="wb-dm-time">'+wbEsc(d.time)+'</div></div>';
    }).join('');
...
return '<div class="wx-msg-row'+(self?' self':'')+'"><div class="wx-msg-avatar" style="background:'+(self?wbCurrentColor():dm.color)+'">'+wbEsc(self?wbCurrentName()[0]:dm.avatar)+'</div><div class="wx-bubble" style="background:'+(self?'#fff3ef':'#fff')+';color:#333;">'+wbEsc(m.text)+'</div></div>';
    }).join('');
